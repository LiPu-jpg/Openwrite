    ...
    def stream(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise RuntimeError(f"LLM 流式调用失败: {model}, 错误: {e}")
    ...
    def stream_complete(
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.retry_count + 1):
            try:
                logger.debug(
                    "Starting stream for model=%s, attempt=%d",
                    model,
                    attempt + 1,
                )
                response = litellm.completion(**call_kwargs)
                ...
                for chunk in response:
                    ...
                    if chunk.choices and len(chunk.choices) > 0:
                        ...
                        if hasattr(delta, "content") and delta.content:
                            chunk_count += 1
                            logger.debug(
                                "Stream chunk %d: %d chars",
                                chunk_count,
                                len(delta.content),
                            )
                            yield delta.content
                logger.debug("Stream completed, total chunks: %d", chunk_count)
                return
            except Exception as e:
                last_error = e
                logger.warning(
                    "LLM 流式调用失败 (model=%s, attempt=%d/%d): %s",
                    model,
                    attempt + 1,
                    self.retry_count + 1,
                    str(e),
        ...
        raise RuntimeError(
            f"LLM 流式调用失败，已重试 {self.retry_count} 次。"
            f"模型: {model}, 最后错误: {last_error}"
        )
