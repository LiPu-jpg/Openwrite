...
受管理后端状态区分为：首次未启动、准备中、就绪、异常退出、恢复中、已取消、已卸载。dsh 运行期间已启动后端异常退出会有上限的退避重启，启动失败也计入该上限；恢复失败不得变成未处理拒绝、不得退出宿主。取消、宿主退出和卸载不复活。外部 `mode: external` 不接管进程。恢复不得重放付费任务。子进程环境为允许列表，不是 `process.env` 整包复制。
