...
商城扫描器将 DTO 测试中的长英文占位字符串识别为凭据字面量；测试现使用明确的短假值 `test-only` / `test-embedding`，继续验证敏感字段不会进入 DTO。此处没有真实密钥，也没有删除测试。扫描上限仍是 150 个源码文件，仓库文件更多时报告保持“扫描不完整”；子进程和编辑器运行时代码能力继续披露，不能把有界扫描称为完整安全审核。
