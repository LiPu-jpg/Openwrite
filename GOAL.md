...
>
> Last updated: 2026-08-31 (contract hardening, shared schemas, epoch tests, schema codegen)
>
...
1. Introduce a React component/E2E runner (vitest + Playwright) for: secret non-echo after model CRUD, graph/DAG refresh after mutations, SSE-fallback polling, mobile interactions.
2. Separately collect 10–20 representative human chapter labels, choose defensible thresholds, set calibration to `calibrated`, and only then enable the production gate.
3. Optional refactor: split `studio_application.py`, bridge `tools.ts`, `AssetsView`, `GraphView`, large CSS.
4. After full acceptance, remind the user to delete the dedicated temporary OpenRouter key.
