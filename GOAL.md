...
>
> Last updated: 2026-08-30 (contract hardening, shared schemas, epoch tests)
>
...
| Shared machine-readable schema | partial | Six JSON Schemas under `OpenWrite/contracts/` (review-v2 decision, review manifest v2, delivery manifest v2, delivery stage v2, benchmark v1, model-profile surface v1) define required fields, enums, number ranges, patterns, credential `disallowed` lists, and additionalProperties policy; Python linter `tools/schema_lint.py` + 17 parity tests (`test_contract_schema_parity.py`); JS mirror `schema-lint.mjs` runs the same fixture matrix in `npm run test:contracts` — verdict parity proven on both sides | Full codegen (types + validators generated from schema for both languages) not yet done; hand-written validators remain the runtime path until codegen lands |
...
| Frontend automated tests | partial | Deterministic Node tests now cover the epoch-invalidation pure functions (`workbench-epochs.ts` via `npm run test:epochs`, 8 tests: assets/manuscript/outline → graph derivation, tasks→DAG, benchmark/models no-graph, SSE≡polling identity, refresh trigger set); JS contract parity covers bridge compaction, dog v2 authority, schema matrix | React component/E2E runner still absent — browser QA remains manual evidence |
...
1. Generate Python + TypeScript types/validators from `contracts/*.schema.json` (codegen) so the hand-written validators become schema-derived; keep `CONTRACT_INVALID` as the runtime error code.
2. Introduce a React component/E2E runner (vitest + Playwright) for: secret non-echo after model CRUD, graph/DAG refresh after mutations, SSE-fallback polling, mobile interactions.
3. Separately collect 10–20 representative human chapter labels, choose defensible thresholds, set calibration to `calibrated`, and only then enable the production gate.
4. Optional refactor: split `studio_application.py`, bridge `tools.ts`, `AssetsView`, `GraphView`, large CSS.
5. After full acceptance, remind the user to delete the dedicated temporary OpenRouter key.
