...
| Installation and development process lifetime | done | No model call during installation; nine isolated lifecycle tests; real rc.7 CLI/pnpm 9.15.9 offline installs and repairs two temporary profiles, exactly one mount per plugin |
...
| Unified offline gate and CI definition | done | Final parent `npm run check` exits 0: build, doctor, 23 maintenance tests, real CLI profile smoke, 27 preset rows/16 skills, both smokes, 14 epochs, 71 components and shared contracts; CI YAML validates, remote CI and live browser QA not run |

This phase does not publish/install into the real user profile, call models,
modify manuscript/production gates, or claim completion of remaining backend
or browser acceptance work. The historical acceptance checklist below applies
to its dated review-v2 milestones; it is not fresh proof for every current change.
