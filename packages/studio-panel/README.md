...
- **写作**（id `overview`，order 19）：**原生工具箱条 + 三分段切换（总览 ⇄ 正文 ⇄ 审稿）+ 单一内嵌 Studio**——原「总览」「正文」「审稿」三个 tab 合并：同一 iframe 经 hash 片段导航在 仪表盘 / 章节编辑器 / 审稿工作台 间切换（不整页重载，各面状态保留）。工具箱补齐 Studio「工具与设置」抽屉在 dsh 侧的最后缺口——导出（md/txt/epub，经二进制安全代理下载，Content-Disposition 文件名）、同步项目、导入章节（选文件 → 预览切章计划/冲突检测 → 确认导入，支持起始章节号与强制覆盖；TXT/Markdown）。**研究 tab 增加发起面板**：输入研究问题 → 提交为托管后台任务（type=research，进度在任务 tab），运行环境未就绪/凭据缺失时透传 setup_hint 与可恢复错误。
...
- **任务**（id `tasks`，order 24）：后台任务中心 —— 状态过滤片（运行中/待确认/排队/已中断/失败/已取消/已完成）、类型徽章（写章/评审/连写/修订/风格源/参考库/导入/研究）、phase 进度信号、失败任务的错误消息；每 5s 轮询（页面隐藏时跳过，切换标签卸载即停止），行悬停操作：运行/排队可取消、可恢复失败可重试（模式白名单 `tasks/{id}/cancel|retry`）。**评审结果可见化**：完成的 chapter_review 行内嵌分数徽章（分档着色），点击展开问题清单（severity 分级着色 + 维度 + 摘要，最多 10 条）——conductor 经 HTTP 跑的评审与 agent 工具跑的同样可见。
...
- **搜索**（id `search`，order 27）：项目资料原生搜索——防抖（350ms）搜索框 + 范围下拉（all/outline/core/characters/settings/continuity/chapters/sources，即服务端 canonical scopes），结果列表显示标题、scope/category 徽章、`path:line`、snippet 内查询词高亮（`<mark>`；payload 无偏移量，用响应里的归一化 query 在客户端重匹配），有加载/空态/服务端 warning 提示。唯一的交互式（非变更）视图。**点击即读**：任意结果打开内联预览（GET /api/document 统一取正文，命中行±2 行带行号高亮，返回按钮回列表）。
...
安装走 profile bundle 路径（与 `@dsh-novel/openwrite-bridge` 相同）：`scripts/install.sh` 把本包 `pnpm add -w` 进 `~/.dsh/profiles/web/` 并追加到 `dsh.profile.bundles`；`cordis.patch.yml` 通过 `--patch` 叠加层或 profile 的 `dsh.bundle.patch` 把插件挂进 cordis 配置树。加载后，打开任意会话即可在头部标签栏看到「聊天 / 轨迹 / 写作 / 大纲 / 资产 / 任务 / 图谱 / 研究 / 搜索」。
...
- **字段级自动保存**（AssetEditor）：name/summary/aliases/tags/scalars/lists 在 blur 时若相对加载基线有变化，即以单键 `data` 走 `assets/update` 自动提交；响应携带的 `asset.revision` 静默接续乐观锁（不重渲染、焦点保留）；409 冲突拉横幅并以 epoch 重挂服务端真值。related 行与 Vditor 正文仍走显式「保存」。注意服务端合并语义：**空字符串会被白名单合并跳过**（无法把字段清空为 ''，只能改写为非空值），编辑器据此跳过"清空即保存"。
